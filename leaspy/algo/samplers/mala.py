...
class MALA(AbstractSampler):
    ...
    def __init__(self, info, n_patients):
        ...
        if info["type"] == "population":
            ...
            self.std = 0.01 * torch.ones(self.shape) # TODO hyperparameter here
    ...
    def _proposal(self, val, std, derivative):
        """
        ...
        return val + distribution.sample() + std * std * derivative.reshape(self.std.shape) / 2
...
#        index = [e for e in itertools.product(*[range(s) for s in shape_current_variable])]
        ...
        if self.moments is None or not self.name in self.moments["attachment"][1]:
            ...
            self.moments["attachment"] = model.compute_moments_model(data.timepoints, individual_parameters, order=1,
                                                                     selected_variables = [self.name],
                                                                     attribute_type='MCMC')
            self.moments["regularization"] = model.compute_moments_regularization(realizations, order=1,
                                                                                  selected_variables=[self.name],
                                                                                  attribute_type='MCMC')
        ...
        new_val = self._proposal(realization.tensor_realizations, self.std, -derivative[self.name])
